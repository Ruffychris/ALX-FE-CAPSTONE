...
function App() {
  ...
  // 1. LANDING VIEW (Your first wireframe)
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#0f172a] via-[#1e3a8a] to-[#0d9488] flex flex-col items-center justify-between p-10 text-white">
        <div className="mt-20 text-center">
          ...
          <h1 className="text-4xl font-bold tracking-tight">WeatherDash</h1>
          <p className="text-white/60 mt-2">Check the weather anywhere</p>
        </div>
        
        <div className="w-full space-y-4">
          <button 
            onClick={() => setView('search')}
            className="w-full bg-white/10 border border-white/20 p-5 rounded-3xl text-xl font-light text-left flex justify-between items-center"
          >
            Search for a city... <span>🔍</span>
          </button>
          
          <button 
            onClick={handleGeolocation}
            className="w-full bg-teal-500 p-5 rounded-3xl text-xl font-bold flex items-center justify-center gap-3 shadow-lg"
          >
            📍 Use My Location
          </button>
        </div>
  ...
  // 2. SEARCH VIEW (Your second wireframe)
  if (view === 'search') {
    return (
      <div className="min-h-screen bg-slate-900 p-6 text-white animate-in slide-in-from-bottom duration-300">
        <button onClick={() => setView('landing')} className="text-3xl mb-6">←</button>
        <form onSubmit={handleSearchSubmit} className="relative mb-10">
          <input 
            autoFocus
            type="text" 
            placeholder="Search city..." 
            className="w-full p-5 pl-14 rounded-3xl bg-white/10 border border-white/20 outline-none focus:ring-2 focus:ring-teal-400"
            value={searchQuery}
        ...
        <h3 className="text-white/40 uppercase tracking-widest text-xs font-bold mb-4">Recent Searches</h3>
        <div className="space-y-3">
          {recentSearches.map((city) => (
            <button 
              key={city}
              onClick={() => fetchWeather(city)}
              className="w-full bg-white/5 p-4 rounded-2xl text-left flex justify-between items-center"
            >
              {city} <span className="text-white/20">→</span>
            </button>
          ))}
        </div>
      </div>
  ...
  // 3. DASHBOARD VIEW (Your third wireframe)
  return (
      ...
      {loading ? (
        ...
        <div className="animate-in fade-in duration-500">
          <div className="flex justify-between items-center mb-10">
            <button onClick={() => setView('search')} className="text-2xl font-light">🔍</button>
            <h2 className="text-xl font-bold">{weather?.name}</h2>
            <button onClick={handleGeolocation} className="text-2xl font-light">📍</button>
          </div>

          <div className="flex flex-col items-center text-center mb-10">
            <h1 className="text-9xl font-extralight tracking-tighter mb-2">{Math.round(weather?.main.temp)}°</h1>
            <p className="text-teal-300 uppercase tracking-[0.2em] font-medium">{weather?.weather[0].description}</p>
            <img 
          ...
          <h3 className="font-bold mb-4 px-2">Next 5 Days</h3>
          <div className="flex overflow-x-auto gap-4 pb-4 no-scrollbar">
            {forecast.map((day, index) => (
              <div key={index} className="min-w-[100px] bg-white/5 p-4 rounded-3xl flex flex-col items-center border border-white/10">
